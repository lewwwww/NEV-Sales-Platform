      ...
      <el-form class="center-form-pv" :style='{"width":"90%","margin":"0 auto 20px"}' :inline="true"
        ...
        <el-row :style='{"display":"block"}'>
          ...
          <div :style='{"margin":"0 30px 0 0","display":"inline-block"}' class="select">
            <label
                :style='{"margin":"0 10px 0 0","color":"#333","textAlign":"center","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}'
                class="item-label">保养状态</label>
            <el-select @change="sfshChange" clearable v-model="searchForm.sfsh" placeholder="保养状态">
        ...
        <el-row :style='{"margin":"20px 0","display":"flex"}'>
          <el-button
              ...
              v-if="isAuth('baoyangdingdan','新增')" type="success" @click="addOrUpdateHandler()">新增
          </el-button>
          <el-button
              :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"rgba(222, 239, 255, 1)","width":"auto","fontSize":"14px","height":"40px"}'
              v-if="isAuth('baoyangdingdan','每月销售额统计')" type="warning" @click="chartDialog1()">每月销售额统计
          </el-button>
        </el-row>
      </el-form>
...
<!--        <el-table-column :resizable='true' type="selection" align="center" width="50"></el-table-column>-->
        ...
        <el-table-column :resizable='true' :sortable='false' prop="ispay" label="是否支付">
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.row.ispay!='已支付' && isAuth('baoyangdingdan','支付') && scope.row.sfsh=='已保养' " type="text" size="small"
        ...
        <el-table-column :resizable='true' :sortable='false' prop="sfsh" label="审核状态">
          <template slot-scope="scope">
            <span style="margin-right:10px" v-if="scope.row.sfsh=='待保养'">待保养</span>
            <span style="margin-right:10px" v-if="scope.row.sfsh=='已保养'">已保养</span>
          </template>
        </el-table-column>
        <el-table-column width="300" label="操作">
          <template slot-scope="scope">
            <el-button
                ...
                v-if=" isAuth('baoyangdingdan','查看')" type="success" size="mini"
                @click="addOrUpdateHandler(scope.row.id,'info')">详情
            </el-button>
            <el-button
                :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"#333","borderRadius":"4px","background":"rgba(230, 242, 254, 1)","width":"auto","fontSize":"14px","height":"32px"}'
                v-if=" isAuth('baoyangdingdan','修改')" type="primary" size="mini"
                @click="addOrUpdateHandler(scope.row.id)">修改
            </el-button>

            <el-button
                :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"#333","borderRadius":"4px","background":"rgba(230, 242, 254, 1)","width":"auto","fontSize":"14px","height":"32px"}'
                v-if="isAuth('baoyangdingdan','审核')  && scope.row.sfsh=='待保养'  " type="info" size="mini" @click="shDialog(scope.row)">保养</el-button>
            <el-button
                :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"#333","borderRadius":"4px","background":"rgba(230, 242, 254, 1)","width":"auto","fontSize":"14px","height":"32px"}'
                v-if="isAuth('baoyangdingdan','删除')  && scope.row.sfsh != '已保养'" type="danger" size="mini" @click="deleteHandler(scope.row.id)">
              删除
            </el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog title="保养" :visible.sync="sfshVisiable" width="50%">
      <el-form ref="form" :model="form" label-width="80px">
        <el-form-item label="保养状态">
          <el-select v-model="shForm.sfsh" placeholder="保养状态">
            <el-option label="待保养" value="待保养"></el-option>
...
export default {
  ...
  methods: {
    ...
    peisongdingdanCrossAddOrUpdateHandler(row, type, crossOptAudit, statusColumnName, tips, statusColumnValue) {
      ...
      this.$storage.set('crossTable', 'baoyangdingdan');
    ...
    payHandler(row) {
      this.$storage.set('paytable', 'baoyangdingdan');
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(() => {
        ...
        this.$http({
          url: `baoyangdingdan/value/xiadanshijian/yingfujine/月`,
          method: "get",
        }).then(({data}) => {
          if (data && data.code === 0) {
            let res = data.data;
    ...
    init() {
      this.sfshOptions = "待保养,已保养".split(',');
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
