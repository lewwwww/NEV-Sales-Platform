<template>
	<div class="addEdit-block" :style='{"padding":"30px"}' style="width: 100%;">
		...
		>
			<template >
        <el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="select" label="订单车辆" prop="dingdanbianhao">
          <el-select @change="chemingChange" :disabled="ro.weixiucheliang" v-model="ruleForm.dingdanbianhao" placeholder="请选择订单车辆" >
            <el-option
                v-for="(item,index) in orderInfoOptions"
                v-bind:key="index"
                :label="item.cheming"
                :value="item.dingdanbianhao">
            </el-option>
          </el-select>
        </el-form-item>
        <el-input v-model="ruleForm.cheming" placeholder="车名" v-show="false" clearable  :readonly="ro.cheming"></el-input>
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.yingfujine" label="应付金额" prop="yingfujine">
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="date" v-if="type!='info'" label="预约时间" prop="xiadanshijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.xiadanshijian" label="预约时间" prop="xiadanshijian">
					<el-input v-model="ruleForm.xiadanshijian" placeholder="预约时间" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="预约备注" prop="xiadanbeizhu">
					<el-input v-model="ruleForm.xiadanbeizhu" placeholder="预约备注" clearable  :readonly="ro.xiadanbeizhu"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else class="input" label="预约备注" prop="xiadanbeizhu">
					<el-input v-model="ruleForm.xiadanbeizhu" placeholder="预约备注" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
    ...
    chemingChange(dingdanbianhao){
      this.$http({
        url: `qichedingdan/infobianhao/${dingdanbianhao}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
          let catOrderInfo = data.data;
          this.ruleForm.cheming = catOrderInfo.cheming;
          this.ruleForm.pinpaileibie = catOrderInfo.pinpaileibie;
        } else {
      ...
      // 获取订单信息
      this.$http({
        url: `qichedingdan/selectList`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
          this.orderInfoOptions = data.data;
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `baoyangdingdan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.baoyangdingdanCrossAddOrUpdateFlag = false;
						 this.parent.search();
